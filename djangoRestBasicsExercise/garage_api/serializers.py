...
class PartManufacturerWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if manufacturer_id is not None:
            try:
                attrs['manufacturer_instance'] = Manufacturer.objects.get(id=manufacturer_id)
            except Manufacturer.DoesNotExist as exc:
                raise serializers.ValidationError(
                    {"id":"Manufacturer with the given ID does not exist."}
                ) from exc
            return attrs
...
class PartWriteSerializer(serializers.ModelSerializer):
    ...
    def resolve_manufacturer(manufacturer_data):
        ...
        return Manufacturer.object.create(**manufacturer_data.get('manufacturer_data'))
    ...
    def update(self, instance, validated_data):
        manufacturer = validated_data.pop('manufacturer_data')
        cars = validated_data.pop('cars', [])
